...
//Start This is used to GEt the materilas  and caluculate the bill that is going to be inserted 
	 ...
	 $sqlm = "SELECT * FROM `material_subcat` INNER JOIN material_cat on material_subcat.cat_id=material_cat.cat_id inner join material_sub_attri on material_sub_attri.sub_id=material_subcat.sub_id inner JOIN material_per_price on material_per_price.sub_attri_id=material_sub_attri.sub_attri_id WHERE material_sub_attri.sub_attri_id = '".$Material."'";  
      ...
      if (filter_var($MPrice1, FILTER_VALIDATE_INT)) {
      ...
      if (filter_var($MPrice1, FILTER_VALIDATE_INT) AND filter_var($MPrice2, FILTER_VALIDATE_INT) AND filter_var($MPrice3, FILTER_VALIDATE_INT)) {
...
//This is used to GEt the procedures and caluculate the bill that is going to be inserted 
      ...
      if (filter_var($Price1, FILTER_VALIDATE_INT) AND filter_var($Price2, FILTER_VALIDATE_INT)) {
      ...
      if (filter_var($Price1, FILTER_VALIDATE_INT) AND filter_var($Price2, FILTER_VALIDATE_INT) AND filter_var($Price3, FILTER_VALIDATE_INT) AND filter_var($Price4, FILTER_VALIDATE_INT) AND filter_var($Price5, FILTER_VALIDATE_INT) AND filter_var($Price6, FILTER_VALIDATE_INT) AND filter_var($Price7, FILTER_VALIDATE_INT) AND filter_var($Price8, FILTER_VALIDATE_INT)) {
      ...
      {  
       ...
       $procedures=$row4['cat_name'].' | '.$row4['sub_name'].' | '.$row4['attribute_name'].' | '.$row4['abbrev'].' | '.$row4['other_name'];
        ...
        if (filter_var($Price1, FILTER_VALIDATE_INT)  AND filter_var($Price2, FILTER_VALIDATE_INT) AND filter_var($Price3, FILTER_VALIDATE_INT) AND filter_var($Price4, FILTER_VALIDATE_INT)AND filter_var($Price5, FILTER_VALIDATE_INT) AND filter_var($Price6, FILTER_VALIDATE_INT) AND filter_var($Price7, FILTER_VALIDATE_INT) AND filter_var($Price8, FILTER_VALIDATE_INT) AND filter_var($Price9, FILTER_VALIDATE_INT) AND filter_var($Price10, FILTER_VALIDATE_INT)) {
      ...
      {  
       ...
       $procedures=$row5['cat_name'].' | '.$row5['sub_name'].' | '.$row5['attribute_name'].' | '.$row5['abbrev'].' | '.$row5['per_name'];
         ...
         if (filter_var($Price1, FILTER_VALIDATE_INT) AND filter_var($Price2, FILTER_VALIDATE_INT) AND filter_var($Price3, FILTER_VALIDATE_INT)) {
...
//This is used to check the all values that is going to be inserted 
      ...
      mysqli_query($conn, "INSERT INTO `bill`(`bill_id`, `date`, `location`, `clinic`, `dentist`, `status`, `cheif_complaint`, `sign_symp`, `diagnosis`, `procedure_name`, `material`, `prescription`, `patient_id`, `amount`,`total_amount`, `payment`) VALUES ('', NOW(), '$Location', '$clinic', '$dentist', '$Status', '$cheifcom', '$signandsym', '$Diagnosis', '$procedures', '$materials', '$Prescription', $p_id, $total, $total, '0')") or die("NO");
      } else {
      	# code...
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active"><a href="record1.php?p_id=<?php echo $_GET['p_id'];?>">Patient Record</a></li>
        ...
        <div class="card card-num">
            <table id="example" class="display responsive nowrap" style="width:100%">
			  ...
			  if (isset($_GET['p_id'])) {
			  	...
			  	if(!$num){
			  	    echo '<h1> No Work Done is Applied on This Patient. Please Apply Work Done.</h1>';
			  	    $bool=1;
